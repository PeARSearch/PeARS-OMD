...
# simulate trying to access the site list while the gateway is down
def test_allsites_gateway_down(client, utils):
...
def test_subscribe_to_site_gateway_down(client, utils):
    ...
    with client: # needed for accessing session
        ...
        flash_type, flash_message = flask_session["_flashes"].pop()
    
    # set the gateway URL back to the real one
    utils.unset_fake_gateway_path()

    assert flash_type == "message" and flash_message.startswith("A request exception occurred:")
...
# simulate trying to unsubscribe from a site while the gateway is down
# N.B.: needs to be run *before* test_unsubscribe_from_site, do not change the testing order
def test_unsubscribe_from_site_gateway_down(client, utils):
    omd_session_token = utils.get_omd_session_id()
    client.set_cookie("OMD_SESSION_ID", omd_session_token)
...
# simulate trying to unsubscribe from all sites (see also test_update_subscriptions_unsubscribe_all) while the gateway
def test_update_subscriptions_unsubscribe_all_gateway_down(client, utils):
    omd_session_token = utils.get_omd_session_id()
    client.set_cookie("OMD_SESSION_ID", omd_session_token)
    
    with client:
        # (re)subscribe to onmydisk (so that we have something to unsubscribe from)
        ...
        # send empty site list to update_subscriptions
        page = client.post(
        ...
        # step 4: unsubscribe (so we have a clean slate for the next test)
        page = client.get("/subscriptions/unsubscribe_from_site?sitename=onmydisk", headers={"Token":AUTH_TOKEN}, follow_redirects=True)
        flash = flask_session["_flashes"].pop()
        assert flash == ('message', 'Unsubscribed from site onmydisk')
